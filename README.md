...
|-------------------|---------|----------------------------------------------|------------------------------------------------------------------------------------------------------------------------------------------|
| `model_name`      | string  | `tts_models/en/ljspeech/vits`                | The Coqui TTS model to use. A list of available models can be found by running `tts --list_models`.                                      |
| `language`        | string  | `''` (empty)                                 | The two-letter language code (e.g., `en`, `es`, `fr`) for multi-lingual models like XTTS. Should be left empty for single-language models. |
| `device`          | string  | `cpu`                                       | The device to run inference on. Can be `cuda` for NVIDIA GPUs or `cpu`.                                                                  |
| `reference_wav`   | string  | `''` (empty)                                 | Path to a WAV file used for voice cloning with `xtts` models. This parameter is required when using an `xtts` model.                      |
| `sample_rate`     | integer | `24000`                                      | The audio sample rate for playback. **This should match the native sample rate of the TTS model for the best quality.** Most `xtts` and `bark` models use **24000** Hz, while many `vits` models use **22050** Hz. If audio sounds speed up or distorted, this is the first parameter to check. The correct rate is often printed to the console when the model first loads. |